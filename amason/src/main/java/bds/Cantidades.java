...
public class Cantidades {
	...
	public Cantidad[] realizar_pedido(Producto[] aProductos, int[] aCantidades, Cliente aCliente) throws PersistentException {
		...
		for (int i = 0; i < arrayCantidad.length; i++) {
			System.out.println(""+ arrayCantidad[i].getId() + " idproducto"+ arrayCantidad[i].getContiene_un().getId() + " cantidad"+  arrayCantidad[i].getCantidad() );
		}
